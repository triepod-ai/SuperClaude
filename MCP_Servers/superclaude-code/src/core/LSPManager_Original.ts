...
import { SecureProcessManager } from '@superclaude/shared';
import { 
  createConnection,
  ProposedFeatures,
  InitializeParams,
...
export class LSPManager extends EventEmitter {
  ...
  private documents: ManagedMap<string, TextDocument>;
  ...
  constructor() {
    ...
    this.resourceMonitor = new ResourceMonitor({
      ...
      memoryThresholdBytes: 300 * 1024 * 1024 // 300MB threshold
    });
    ...
    this.documents = new ManagedMap('lsp-documents', this.resourceMonitor, {
      maxSize: 1000,
    ...
    logger.info('LSPManager initialized with resource management');
  }
  ...
  private setupResourceMonitoring(): void {
    this.resourceMonitor.on('memory_threshold_exceeded', (data) => {
      logger.warn('Memory threshold exceeded in LSPManager', data);
      this.performMemoryCleanup();
    });

    this.processManager.on('process_error', (data) => {
      logger.error('LSP server process error', data);
      this.emit('server-error', data.processId, data.error);
    });

    this.processManager.on('process_exit', (data) => {
      logger.warn('LSP server process exited', data);
      this.handleServerProcessExit(data.processId, data.code, data.signal);
    });

    this.processManager.on('process_restart', (data) => {
      logger.info('LSP server process restarted', data);
    });
  }
   ...
   */
  private handleServerProcessExit(processId: string, code: number, signal: string): void {
    // Find and clean up the server
  ...
  private performMemoryCleanup(): void {
    // Close least recently used documents
    const documentEntries = Array.from(this.documents.entries());
    const documentsToRemove = Math.floor(documentEntries.length * 0.3);
    
    // Sort by access time (this would require tracking access times)
    // For now, remove oldest documents
    for (let i = 0; i < documentsToRemove; i++) {
      const [uri] = documentEntries[i];
    ...
    logger.info('LSPManager memory cleanup completed', { documentsRemoved: documentsToRemove });
  }
  ...
  async startServer(language: SupportedLanguage, workspaceRoot: string): Promise<void> {
    ...
    try {
      ...
      const server = new LSPServer(language, config, workspaceRoot, this.processManager);
      await server.start();
  ...
  async cleanup(): Promise<void> {
    try {
      ...
      // Cleanup resource monitor and cache manager
...
class LSPServer extends EventEmitter {
  ...
  constructor(
    ...
    super();
    this.processId = `lsp-${language}-${Date.now()}`;
  }

  async start(): Promise<void> {
    try {
      // Spawn LSP server process using secure manager
      this.process = await SecureProcessManager.spawnSecure({
        command: this.config.command,
        args: this.config.args,
        timeout: 60000, // 60 seconds for LSP initialization
        maxMemory: 1024, // 1GB memory limit for LSP servers
      ...
      this.processManager.registerProcess(this.processId, this.process, {
        autoRestart: false, // Let LSPManager handle restarts
        maxRestarts: 0,
        timeout: 30000
      });
      ...
      // Setup error handling through ProcessManager events
      // (ProcessManager will handle process events and emit through LSPManager)
  ...
  async stop(): Promise<void> {
    try {
      ...
      // Terminate process through ProcessManager
      await this.processManager.terminateProcess(this.processId, false);
